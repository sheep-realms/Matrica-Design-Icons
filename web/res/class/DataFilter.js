...
class DataFilterConditions {
    ...
    checkForCondition(condition, searchData, item) {
        ...
        function __checkNumberRange(search, value, isHex = false) {
            if (isHex) {
        ...
        function __checkHexNumberRange(search, value) {
            return __checkNumberRange(search, value, true);
        }
        ...
        function __checkVersionRange(search, value) {
            if (String(search).search(/\.\./) !== -1) {
                let min = search.split('..')[0];
                let max = search.split('..')[1];
                min = min === '' ? '0.0.0' : min;
                max = max === '' ? '9999.9999.9999' : max;
                min = __compareVersions(min, value);
                max = __compareVersions(max, value);
                return min <= 0 && max >= 0;
            } else {
                return search === value;
            }
        ...
        } else if (condition.type === 'hex_number') {
            return __checkAllCondition(__checkHexNumberRange);
